...
module.exports = {
    playSong:
        async (message, channel, guild, adapter, song, error) => {
            try {
                if(!servers[guild]) {
                    servers[guild] = {
                        queue:[],
                ...
                player.on(AudioPlayerStatus.Idle, async () => {
                    if (skip == false) {
                        skip = true;
                        module.exports.skipSong(message, guild, (e) => {
                            if(e) {
                                return error(e);
                            }
                        });
    ...
    skipSong:
        async (message, guild, error) => {
            try {
                if(!servers[guild]) {
                    return message.channel.send(`:woman_facepalm: There is nothing to skip`);
                }
